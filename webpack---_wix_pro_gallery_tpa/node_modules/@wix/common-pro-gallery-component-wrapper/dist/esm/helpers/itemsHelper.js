...
var ItemsHelper = /** @class */ (function() {
    function ItemsHelper(galleryWrapper, props, isStoreGallery) {
        ...
        this.preloadedItems = {};
        this.preloadedFullscreenItems = {};
        this.fullscreenItemsDimensions = {};
    ...
    ItemsHelper.prototype.renderInfoElement = function(type, pgItemProps) {
        ...
        return (React.createElement(InfoElement, __assign({}, this.processPgItemPropsForInfoElement(type, pgItemProps))));
    };
    ...
    ItemsHelper.prototype.preloadItem = function(item, itemsCacheKey, itemsDimensionsKey, onload) {
        if (!item || !item.itemId || !item.mediaUrl) {
            return;
        }
        try {
            var id = item.itemId;
            if (this[itemsDimensionsKey][id]) {
                return; // already measured
            }
            if (typeof this[itemsCacheKey][id] !== 'undefined') {
                return;
            }
            this[itemsCacheKey][id] = new Image();
            if (utils.isVerbose()) {
                console.log('Preloading item #' + item);
            }
            this[itemsCacheKey][id].src = item.mediaUrl;
            if (typeof onload === 'function') {
                this[itemsCacheKey][id].onload = function(e) {
                    onload(e);
                };
            }
            return this[itemsCacheKey][id];
        } catch (e) {
    ...
    ItemsHelper.prototype.loadDirectFullscreenItemDimensionsIfNeeded = function() {
        var _this = this;
        if (utils.isSSR()) {
            return;
        }
        var fullscreenState = this.galleryWrapper.state.fullscreen;
        if (fullscreenState.directFullscreenItem &&
            fullscreenState.directFullscreenItem.itemId &&
            this.isDimensionless(fullscreenState.directFullscreenItem)) {
            this.preloadItem(fullscreenState.directFullscreenItem, 'preloadedFullscreenItems', 'fullscreenItemsDimensions', function(e) {
                var ele = e.srcElement;
                var itemId = fullscreenState.directFullscreenItem.itemId;
                var mediaUrl = fullscreenState.directFullscreenItem.mediaUrl;
                _this.galleryWrapper.setState({
                    fullscreen: __assign(__assign({}, fullscreenState), {
                        directFullscreenItem: __assign(__assign({}, fullscreenState.directFullscreenItem), {
                            metaData: __assign(__assign({}, fullscreenState.directFullscreenItem.metaData), {
                                width: ele.width,
                                height: ele.height
                            })
                        })
    ...
    ItemsHelper.prototype.loadItemsDimensionsIfNeeded = function() {
        ...
        itemsWithoutDimensions.forEach(function(item, idx) {
            ...
            _this.preloadItem(item, 'preloadedItems', 'itemsDimensions', function(e) {
                try {
                    if (utils.isVerbose()) {
