		...
		[namespace]: {
			...
			network: {
				...
				importAMDModule: (url, dependenciesMap) => {
					if (platformEnvData.window.isSSR && featureConfig.shouldPreloadPlatformScripts) {
						// adds a preload script tag in the SSR response in order to improve the client side network requests waterfall
						handlers.addScriptToPreloadList(url)
					}
