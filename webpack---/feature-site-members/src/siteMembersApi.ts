...
import { getPageIdByWidgetId } from './platformizedAuthHelper'
	...
	const api = {
		...
		promptForgotPassword(isCloseable: boolean = true): Promise<void> {
			return new Promise((resolve, reject) => {
				...
				const shouldUsePlatformizedAuth = experiments['specs.thunderbolt.usePlatformizedAuth']
				if (shouldUsePlatformizedAuth) {
					const forgotPasswordPageId = getPageIdByWidgetId(viewerModel, TPA_PAGES.FORGOT_PASSWORD)
					if (forgotPasswordPageId) {
						return popups?.open(forgotPasswordPageId)
					}
				}
		...
		async applySessionToken(
			...
			if (experiments['specs.thunderbolt.preventApplySessionTokenInSSR'] && isSSR(browserWindow)) {
				throw NOT_PERMITTED
			}
		...
		async showResetPasswordDialog(token: string) {
			...
			if (shouldUsePlatformizedAuth) {
				const resetPasswordPageId = getPageIdByWidgetId(viewerModel, TPA_PAGES.RESET_PASSWORD)
				if (resetPasswordPageId) {
					return popups?.open(resetPasswordPageId)
				}
			}
